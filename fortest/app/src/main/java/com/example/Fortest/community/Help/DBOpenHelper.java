...
public class DBOpenHelper extends SQLiteOpenHelper {
    ...
    public DBOpenHelper(Context context){
        super(context,"db_test2",null,1);  //修改表的时候  一定要修改版本号！！！
        db = getReadableDatabase();
    }
...
//    private String animalQuestion;
    ...
    public void onUpgrade(SQLiteDatabase db, int oldVersion, int newVersion){
        db.execSQL("DROP TABLE IF EXISTS help");
        onCreate(db);
    }
    ...
    public void add(Integer _id,String animalName, String animalType, String animalQuestion){
        db.execSQL("INSERT INTO animal(u_id,animalName,animalType,animalQuestion) VALUES(?,?,?,?)",new Object[]{_id,animalName,animalType,animalQuestion});
    }
    //todo 修改参数  参照以上
...
//    }
     ...
     */
    //todo 需要加上——id??
    public ArrayList<Help> getAllData(){

        ArrayList<Help> list = new ArrayList<Help>();
        //下面打开一个游标
        Cursor cursor = db.query("help",null,null,null,null,null,"u_id DESC");
        while(cursor.moveToNext()){
            ...
            if(forid==cursor.getInt(cursor.getColumnIndex("u_id"))){
                ...
                String animalQuestion= cursor.getString(cursor.getColumnIndex("animalQuestion"));

                list.add(new Help(animalName,animalType,animalQuestion));
            }
            forid=-1;
        }
