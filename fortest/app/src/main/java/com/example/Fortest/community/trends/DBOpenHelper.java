...
public class DBOpenHelper extends SQLiteOpenHelper {
    ...
    public DBOpenHelper(Context context){
        super(context,"db_test3",null,1);  //修改表的时候  一定要修改版本号！！！
        db = getReadableDatabase();
    }
...
//    private String animalImage;
    ...
    public void add(Integer _id,String title, String text, String animalName, String communityName, String animalImage){
        db.execSQL("INSERT INTO animal(u_id,title,text,animalName,communityName,animalImage) VALUES(?,?,?,?,?,?)",new Object[]{_id,title,text,animalName,communityName,animalImage});
    }
    //todo 修改参数  参照以上
...
//    }
     ...
     */
    //todo 需要加上——id??
    public ArrayList<trends> getAllData(){

        ArrayList<trends> list = new ArrayList<trends>();
        //下面打开一个游标
        Cursor cursor = db.query("trends",null,null,null,null,null,"u_id DESC");
        while(cursor.moveToNext()){
            ...
            if(forid==cursor.getInt(cursor.getColumnIndex("u_id"))){
                ...
                String animalImage= cursor.getString(cursor.getColumnIndex("animalImage"));
                list.add(new trends(title,text, animalName,communityName,animalImage));
            }
            forid=-1;
        }
